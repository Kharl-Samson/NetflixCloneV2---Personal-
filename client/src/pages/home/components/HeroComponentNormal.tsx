  ...
  return (
    ...
    <>
      ...
      screenWidth >= 640 && 
        ...
        >
          <p 
            className={`text-white custom-transition-duration-10s movie-title-font-large max-w-[40rem] xl:max-w-[80%] 
              leading-tight ${textAnim ? "text-4xl 801size:text-5xl 2xl:text-7xl" : "text-5xl 801size:text-7xl 2xl:text-8xl"}`}
          >
            {myData?.title || myData?.name || myData?.original_title}
          </p>
          
          <p className={`mt-4 text-white text-base xl:text-xl max-w-[70%] 2xl:max-w-[50%] custom-transition-duration-10s ${textAnim ? "hidden" : "block"}`}>
            {myData?.overview}
          </p>
          ...
          screenWidth >= 640 &&
            <div className="hidden z-20 h-[3.1rem] absolute max-w-[3000px] mx-auto right-0 text-white sm:flex gap-5 items-center mb-[15.8rem]">
              <div 
                className={`h-[50px] w-[50px] border border-solid border-white rounded-full items-center cursor-pointer
                  justify-center transition duration-400 hover:bg-gray-700 hover:bg-opacity-35 ${(showVideo || videoEnded) ? "flex" : "hidden"}`} 
              ...
              <div className="custom-transparent-bg border-l-[3px] border-white h-[50px] w-[125px] flex items-center">
                <span className="text-[1.3rem] ml-4">{age}+</span>
              </div>
            </div>
          }
        </div>
