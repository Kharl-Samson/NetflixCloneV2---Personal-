  ...
  return (
    <>
        ...
        screenWidth > 800 && 
            <div 
                style = {{
                    backgroundImage: `linear-gradient(0deg, rgba(0, 0, 0, 0) 0%, rgba(0, 0, 0, 0) 100%), url(${import.meta.env.VITE_BASE_IMAGE_URL}${myData?.backdrop_path})`
                }}
                className={`max-h-[90rem] hidden 801size:block absolute max-w-[3000px] mx-auto w-full h-[50rem] 951size:h-screen bg-center custom-transition-duration-10s
                bg-cover bg-black bg-opacity-50 image-inline-shadow z-10 ${showVideo ? "opacity-0" : "opacity-100"}`}
            >
            </div>
        }
        ...
        {/* Video Player */
        screenWidth > 800 &&
            <div className={`max-h-[90rem] hidden absolute max-w-[3000px] mx-auto top-0 w-full h-[50rem] 951size:h-screen overflow-hidden z-0 ${showVideo && "801size:block"}`}>
                <YoutubePlayer
                    id = "youtubePlayer"
                    videoId = {trailerData} 
                    duration = {5000}
        ...
        >
            <p 
                className={`mt-[-5rem] text-white custom-transition-duration-10s
                    ...
                    leading-tight ${textAnim ? "text-5xl xl:text-7xl" : "text-7xl xl:text-8xl"}`}
            >
        ...
        {/* For hiding controls */
        screenWidth > 800 &&
            <div className="hidden 801size:block absolute max-w-[3000px] mx-auto h-[5rem] w-full bg-custom-color-hero-1 top-[45rem] 951size:top-[100vh] z-20"></div>
        }

        {/* For shadowing */
        screenWidth > 800 &&
            <div className="z-20 hidden 801size:block shadowing-hero bottom-[17rem] 951size:bottom-[-2rem] 2xl:bottom-[-6rem]"></div>
        }

        {/* Age and Sounds Controller */}
        <div className="hidden z-20 absolute max-w-[3000px] mx-auto right-0 text-white bottom-[33rem] 951size:bottom-[17rem] 2xl:bottom-[15rem] 801size:flex gap-5 items-center disable-highlight">
            <div 
